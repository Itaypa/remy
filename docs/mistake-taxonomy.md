...
|---|---------|-----------------|------------------------------|--------|
| M1 | **Kitchen-sink sessions** — one session for many unrelated topics | `/clear` between unrelated tasks; one topic per session | No clean topic proxy yet; long-lived session + repeated compacts is the nearest signal | partial (`auto-compact`) |
...
| M7 | **Missing or bloated CLAUDE.md** | Short CLAUDE.md of underivable facts; prune regularly | Startup context tax (partial); CLAUDE.md byte stat at SessionStart (2c) for the direct signal | partial: `context-tax` → 2c |
...
| M13 | **Permission fatigue** — click-through approvals, or blanket skip-permissions | Allowlists for known-safe commands; sandbox for autonomy | Permission events are not exposed to hooks today; host-adapter dependent | not detectable (today) |
