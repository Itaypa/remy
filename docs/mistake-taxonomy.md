...
| M16 | **Dirty git state** — no clean baseline to diff or recover to | Clean state per session; checkpoints for risky work | The old blocker text was stale: the probe already exists (`gitStatus` in `cli/src/index.ts`, ≤10ms, run every statusline tick) and a bucketed int needs no schema change. Measured instead, and it fails on the numbers — see trend-watch sweep 4 | **rejected as a rule — measured**; hints line shipped |
