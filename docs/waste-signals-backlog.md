...
| S6 | **Scattered tool failures** — high failure rate without consecutive runs | Each failed call pays a full round trip + error output + recovery turn; scattered failures signal a broken environment (missing dep, wrong cwd) that one fix would end. `retry-loop` only catches *consecutive* identical failures | `tool_fails / tool_calls ≥ 25%` with ≥12 calls in a session (both already on the session row) | new tip `fix-env-once` | **blocked on collection, not unblocked** — measured 2026-08-06. The counter it needs only started working on 2026-08-04 (`bcbdafc`, which registered `PostToolUseFailure`), so 8 of 11 eligible sessions predate it and their zeros mean "not counted", not "no failures". Exactly **one** session has post-fix failure data, at a 1.6% rate (10 of 629); every candidate threshold from 10% to 30% fires on 0 of 11. Unpark at ≥10 sessions recorded after 2026-08-04 |
...
| S9 | **Permission-prompt churn** (taxonomy M13) | Every approval prompt is an interruption, and a denied call is a full round trip that usually gets re-issued — the fix is a one-line allowlist entry the user never makes because nobody counts the prompts | Register the **`PermissionRequest`** hook (it exists now — see the hook-surface note below) and count events per session, bucketed by the tool name we already whitelist. Nothing new is stored beyond a count and an existing enum | ~~`allowlist-the-routine`~~ | **dropped 2026-08-07 — the host ships both halves.** `/fewer-permission-prompts` scans transcripts for common read-only Bash/MCP calls and writes the allowlist itself. Four independent kills: our detection is fenced off by our own design (`PermissionRequest` is deliberately unregistered because stdout there is an allow/deny decision), the host's scan sees the prompts we structurally cannot, interruptions carry no 🪙 so it could never win `promoteNext`, and `/doctor` additionally offers to pre-approve frequently denied read-only commands. Residue: one hints line |
...
| S14 | **Subagent spend is invisible** — `isSidechain` is dead code on this host and worker transcripts moved to their own tree, so REMY's token totals silently omit every delegated agent | Measured locally: **15.1% of all billable tokens invisible**, median 13.4% per delegating session, worst **55.7%**. Worse, the numbers on one screen disagree — `tool_calls` (hooks) includes subagents, tokens (transcript) don't, `cost_usd` (host) does | Walk `<transcript dir>/<session-id>/subagents/agent-*.jsonl` at SessionEnd only; dedupe by `message.id`; never open `.meta.json` (it carries the task prompt). Full spec: [S14](specs/S14-subagent-spend.md) | — (collection only, no tip) | **shipped** — collection only, SessionEnd-gated; verified on a real session (11 workers, 256 tool calls, 897k cache-write, all Opus) |
| S15 | **`delegate-tier`** — workers inherit the top tier nobody chose | Worker `model` is unset on 24 of 44 local agents; pinning workers to a cheaper tier measured 37% fewer tokens at under half the wall-time | Needs S14's columns plus a week of data to calibrate. Fires on inherited defaults only, never on the choice to delegate | new tip `delegate-tier` | **blocked** — needs S14 data |
| S16 | **`subagent-offload`'s estimate has the wrong sign** | `estSavingsTokens = (reads-10)×1500` books *context relief* as tokens *not spent*, while the delegation it recommends costs ~69k billable at the local median — and that field ranks the coaching queue, so it is mis-prioritized coaching, not a cosmetic error | Either drop the value clause and let the tip stand on window headroom (what its own citation actually claims), or clamp it. Needs a copy decision | — (fix to a shipped tip) | **shipped** — estimate dropped to 0; also fixed the `Math.max` high-water mark that made every downward correction inert, and the report calling such a session "clean" |
| S17 | **Whole-file reads** — `Read` returns an entire large file where a slice was asked for, and the window carries it for every turn after | Locally the largest tool-shaped waste in the corpus: **1.95M tokens of `Read` results**, p95 117,944 chars, worst single result 471,536 (~118k tokens — half a 200k window in one call). Bash, which the writeups blame, is the best-behaved tool here | Result *length* only, from the tool_result branch `transcript.ts` already resolves; nothing new stored. ≥3 results over 8k tokens. Full spec: [S17](specs/S17-read-in-slices.md) | new tip `read-in-slices` | **shipped** — fires on 2 of 24 local sessions (est 478k and 700k) |
| S18 | **Effort drift** — `/effort` is sticky and outlives the hard step it was set for | Sweep 1 rejected this as undetectable; that was wrong. `effort` is a **top-level transcript field**, present on 4,065 of 4,077 deduped main-chain turns (the 12 missing are synthetic/error turns with no output). The prior seat had looked inside `message.usage` | Collect per-session effort mix as counts only. **No rule yet:** 0 of 18 local sessions are all-max and the one mixed session is this repo's own night loop, so a rule calibrated today would fire on its author | — (collection only) | **collecting** — columns shipped 2026-08-06, no rule. Unpark when a session that is NOT this repo's night loop uses max, or when `effort_turns` exceeds max+high (a `low`/`medium` value has appeared). Locally: 1 of 24 sessions uses max, 0 unbucketed turns |
| S19 | **The host already attributes turns** — `attributionSkill`, `attributionMcpServer`, `attributionMcpTool`, `attributionPlugin` ride on assistant entries | 2,042 / 854 / 854 / 142 occurrences locally. This is the "configured half" F1 (idle MCP servers) was parked for in sweep 1, and much of what S12 had to infer | Read from the transcript; the names are free-text-shaped and must be hashed or enum-gated at ingest. Likely unparks F1 outright | — (enabler) | **blocked on the same call as S10/S14** — measured 2026-08-06: 1,562 of 4,066 deduped turns carry attribution, and billable spend attributes cleanly (`skill:night-shift` 1.82M, `skill:doctor` 1.04M, `mcp:Claude Browser` 758k). But attribution is not a *mistake*: the user invoked those skills on purpose. Its only consumer is a ledger view — "where your tokens went" — which is a display decision, and building storage ahead of one is what got S8 deferred. Needs a call on the displayed numbers, then it is a one-cycle build |
| S20 | **Hook-injected context** — hooks that inject content on every event | `SessionStart:startup` injects 170,505 chars ≈ **43k tokens, ~950 every session start**; attachment entries carry `hookName`, `hookEvent`, `durationMs`, `exitCode` | Same resolution as S12: feed `context-tax`'s attribution rather than add a tip. Hook names are free text and need hashing | — (attribution) | **backlog** |
